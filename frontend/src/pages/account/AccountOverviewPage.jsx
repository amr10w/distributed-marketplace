...
const AccountOverviewPage = () => {
  ...
  if (!user) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-xl font-semibold text-gray-700">Please login to view your account</h3>
  ...
  const soldTransactions = transactionsData.filter(
    (t) => t.type === 'PURCHASE' && t.sellerId === user.id
  )
  const purchasedTransactions = transactionsData.filter(
    (t) => t.type === 'PURCHASE' && t.buyerId === user.id
  )
  const depositTransactions = transactionsData.filter(
    (t) => t.type === 'DEPOSIT' && t.userId === user.id
  )
  ...
  const tabs = [
    ...
    { key: 'forsale', label: 'Items For Sale' },
  ]
