    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-2xl font-bold text-green-600">{myItems.filter((p) => p.quantity > 0).length}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <p className="text-sm text-gray-500">Out of Stock</p>
          <p className="text-2xl font-bold text-red-600">{myItems.filter((p) => p.quantity === 0).length}</p>
        </div>
      </div>
      ...
      {myItems.length > 0 ? (
                  ...
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link to={'/seller/items/' + item.id + '/edit'} className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                          Edit
                        </Link>
                        <button onClick={() => handleDeleteClick(item.id)} className="text-red-600 hover:text-red-800 text-sm font-medium">
                          Delete
                        </button>
                      </div>
                    </td>
        ...
        <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
          ...
          <Link to="/seller/items/new" className="bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition">
            Add Your First Item
          </Link>
        </div>
      )}
